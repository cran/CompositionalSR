cv.alfaesf <- function(y, x, a = seq(0.1, 1, by = 0.1), coords, model = "exp", nfolds = 10, size = 1000, folds = NULL) {
  if ( min(y) == 0 )  a <- a[a>0]
  la <- length(a)
  n <- dim(y)[1]
  ina <- 1:n
    ...
    for ( i in 1:la ) {
      ...
      for ( m in 1:nfolds ) {
        ...
        coordstest <- coords[folds[[ m ]][[ 2 ]], , drop = FALSE]
         mod <- alfa.esf( ytrain, xtrain, a[i], coords = coordstrain, model = model, xnew = xtest,
                          coordsnew = coordstest, yb = ytr[folds[[ m ]][[ 1 ]], ] )
          yest <- mod$est
          kl <- ytest * log(ytest / yest)
          kl[ is.infinite(kl) ] <- NA
          kul[m] <- sum(kl, na.rm = TRUE) / dim(ytest)[1]
      }  ##  end  for ( m in 1:nfolds ) {
      kula[i] <- mean(kul)
    }  ##  end  for ( i in 1:la ) {

    apa <- proc.time() - apa
    best <- which( kula == min(kula) )
    opt <- c( min(kula), a[ best ] )
